...
OLS, TWFE ve diğer estimator seçenekleri ayrı sistemler değil, aynı atomun farklı eksen kombinasyonlarıdır. Bu sayede runner, varyans ve raporlama akışları estimator-agnostik kalır.
...
flowchart TD
    ...
    L2 --> L3[L3 Vetted Transform Taxonomy<br/>Kapali sozluk, keyfi kod yok]
    L3 --> L4[L4 Subprocess Reproduction Gate<br/>sandbox + assert_frame_equal]
    L4 --> L5[L5 High-Impact Cleaning Gate<br/>satir dusuren karar = zorunlu insan onayi]
    L5 --> L6[L6 Privacy Routing<br/>private modda no-train/ZDR zorlamasi]
    L6 --> L7[L7 Prompt Guard Detective<br/>Groq Prompt Guard 2 + fail-open log]
```
...
- İlke: Yüksek-etki kararlar (ör. `drop_duplicates`) otomatik geçmez, zorunlu onay kapısına düşer.
...
- Uygulama: [pareto/llm/guardrails.py](pareto/llm/guardrails.py)
- İlke: Llama Prompt Guard 2 (Groq) ile detective tarama; hata durumunda fail-open, log + payload izi bırakır.
